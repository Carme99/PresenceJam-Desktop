...
|-------------------------------------------------------|--------|--------------------------------------------------------------------------------------------------------------------------|
| Spotify Premium ↔ Teams personal                       | ✅     | Spotify calls hit `accounts.spotify.com/api/token` (OAuth2 PKCE) + `/v1/me/player/currently-playing`; Teams calls hit `graph.microsoft.com/v1.0/me/presence/setStatusMessage`. The OAuth rounds are wired in `src-tauri/src/spotify.rs` and `src-tauri/src/teams.rs`. |
...
| Pause-aware backoff (Spotify idle)                     | ✅     | `src-tauri/src/polling/poll_once.rs` — 30 → 60 → 120 → 300 s cap after consecutive `Ok(None)`; resets to 30 s on next observed track. Reduces ~720 calls/day to ~25 when paused. |
...
|--------------------------------------------------------------|--------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Microsoft 365 GCC / DoD / EduTenants / China tenants**      | ⚠ Verify | Microsoft's tenant policies restrict which Graph scopes are granted to non-copilot-managed apps. Personal Microsoft accounts (consumer `@outlook.com`, `@hotmail.com`) work because `Presence.ReadWrite` is granted by default. But work/school tenants often require admin consent for `Presence.ReadWrite` and `offline_access`. If you've tested PresenceJam against a tenant-restricted M365 environment, please file an issue or PR against this file with the result — the maintainer has only verified personal-tier accounts. To check from your end: open `https://developer.microsoft.com/en-us/graph/graph-explorer`, sign in with the relevant account, and confirm `GET /me/presence` returns 200. If it returns 403/401, that tenant won't work out-of-the-box. |
