...
Releases are automated on a pushed `v*` tag or a manual re-cut. `resolve-tag`
validates the tag and release documentation; `verify` reruns the tagged commit's
Linux gate set, including Playwright; only then can the three-OS build, release,
Homebrew, and winget jobs proceed.

```mermaid
flowchart TD
    Trigger["🔔 Trigger: git tag v* && git push --tags<br/>(or workflow_dispatch re-cut)"]
    Resolve["🏷️ resolve-tag<br/>validate existing v* tag"]
    Verify["verify<br/>tagged Linux gates<br/>Rust + Vitest + Playwright"]
    ...
    Trigger --> Resolve --> Verify --> Build --> Release --> Brew
    Release --> Winget
```
...
2. **Tag resolution:** the `resolve-tag` job validates the tag exists and checks
   the required CHANGELOG and feature-state documentation.
3. **Tagged-commit verification:** the `verify` job checks out the resolved tag
   and runs format, clippy, Rust tests, frontend type-check, Vitest coverage,
...
6. **Release + updater manifest (v3.0):** The `release` job generates
...
7. **Distribution:** `homebrew` and `winget` jobs (each consuming the GitHub
   Release artifact) update the tap / open a winget-pkgs PR in parallel.

Two gates guard the tag before a single artifact is built:

- **Tag/version agreement** — the `resolve-tag` job's *Verify version consistency*
  step compares the pushed tag against all six version literals: the three
  manifests plus both lockfile root entries. Drift fails the run. The tag decides
  the version written into `latest.json`, while the binary self-reports the
  version baked in at build time, so a mismatch is a permanent re-offer loop: the
  updater keeps advertising a version the app never becomes, and
  `install_pending_on_exit` sees `staged > current` on every quit. `ci.yml` has
  the PR-time twin (`version-consistency` job), so drift is caught before a tag
  exists to disagree with (#605).
- **Tagged-commit verification** — the `verify` job (`needs: resolve-tag`) checks
  out the resolved tag and reruns format, clippy, all-target Rust tests,
  frontend type-check, Vitest coverage, and the Chromium geometry gate. `ci.yml`
  only triggers on pull requests and pushes to `main`, so without this job the
  exact commit that produces user-facing binaries would never be tested — worst
  of all on a `workflow_dispatch` re-cut. `build:` declares
  `needs: [resolve-tag, verify]`, so a failure blocks all three OS legs without
  burning runner minutes (#606).
