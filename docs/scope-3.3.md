...
- `src-tauri/capabilities/default.json` — 24 permissions, all exercised:
  - `core:*` (window/tray/menu/event), `opener:default` (`open_external_url` via `tauri_plugin_opener`), `autostart:default`, `log:default`, `deep-link:default`, `updater:default`, `notification:default` (+ 3 `allow-*`), `single-instance` (registered via `tauri_plugin_single_instance::init` in `lib.rs`, not a capability entry — correct). No `http:*` entry exists: all Teams/Spotify HTTP goes through Rust `reqwest`, so the webview needs no HTTP permission.
