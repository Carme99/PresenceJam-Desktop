...
On Linux, the shortcut backend is X11-only: Wayland-native windows do not receive X11 grabs. A Wayland-native session without a reachable X11/XWayland display reports both configured shortcuts as unavailable instead of claiming they are active. When X11 is reachable, registration follows the normal plugin path; a desktop that still refuses an individual grab reports the typed failure inline and leaves the other binding working.
