...
They work while the window is hidden. Click a field and press the combination you want — the field records what you press, and the current grab is released while it records so the key is captured instead of fired. **Clear** empties a slot. A combination that cannot be parsed, carries no modifier (see below), or collides with the other slot is refused inline; a desktop that refuses the grab (for example, because another app already owns the combination) shows **Registration failed on this desktop** with the reason, and the other binding — and the rest of the app — keeps working.

On Linux, the shortcut backend is X11-only: Wayland-native windows do not receive X11 grabs, and a Wayland-native session without a reachable X11/XWayland display reports both configured shortcuts as unavailable instead of claiming they are active. The plugin does not expose a worker-liveness handle, so a reachable X11 display is also treated as unverified and fails closed rather than reporting a shortcut as active when its worker may be dead.
