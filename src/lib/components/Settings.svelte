...
// `localConfig` is a Svelte 5 `$state` proxy; `structuredClone` in
  ...
  async function reconnectSpotify() {
    ...
    // #550: a popped-out pane must not start the flow itself. The backend
    // rejects the invoke from a non-main window (#241), and `currentView` is
    // main-window-only anyway — so hand the navigation back to the main
    // window and close this one, the same route goToOnboarding takes.
    if (detached) {
