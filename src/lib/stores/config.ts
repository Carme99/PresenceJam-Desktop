...
/**
 * Polling fields are ts-rs `bigint` on the wire and `u64` in Rust. Every one
 * of them needs the same rounding/normalisation on the way in and out; a
 * hand-edited `config.json` that carries `polling` but omits or nulls one of
 * them used to leave the store holding `undefined`, which `toSavePayload`
 * turned into `NaN` → `null` and Rust's serde rejected (issue #541).
 */
const POLLING_KEYS = [
...
/**
 * `defaultConfig.polling` viewed as a plain map. The ts-rs bindings in
 * `src/lib/types-generated/` are derived (gitignored, regenerated by
 * `cargo test`), so a 4.6 field added on the Rust side cannot be read here by
