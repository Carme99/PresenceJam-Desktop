...
Every change merged after the most recent release lands here until the release cut,
which renames this section to the released version and opens a fresh empty one (see
`docs/RELEASING.md` §2). v5.0.1 carry-overs: `docs/HEADLESS.md`, `docs/API.md`, the
third shortcut slot, the PlaybackSource Windows SMTC compile, the per-file coverage
ratchet config, and the Windows-runtime test fix for `#1009`'s new CI legs.

## [5.0.0] - 2026-09-19

The v5.0.0 audit cut — eleven wave-1 + six wave-3 PRs close 47 issues across commands/polling/onboarding/release-pipeline/tray/lib-shell/presence/gating/outlook/profiles/serve/daemon. New surfaces: a token-guarded localhost HTTP control + event API (--serve), a supervised headless daemon (--daemon), an OS-presentation + desktop-idle presence gate, an Outlook calendar pre-gate with same-poll un-gate, an Outlook working-hours importer, named presence profiles switchable from the tray and CLI, a PlaybackSource trait with SMTC + MPRIS sources for non-Premium users, and a bounded status-decision history with a Dashboard Activity card. The wave-3 carry-overs (docs/HEADLESS.md, docs/API.md, the third shortcut slot, the PlaybackSource Windows SMTC compile) are tracked under [Unreleased] for v5.0.1.
