...
### Changed
- **Warm keychain presence avoids repeated OS probes during config loads (#881).** `config::load_config` reuses a fresh `Present` observation from the warm cache first; a cold or expired `Present` entry, or an `Unavailable` result, falls back to the direct tri-state probe. `Present`, `Absent`, and `Unavailable` semantics remain distinct, and `config::with_keychain_flags` stamps both derived fields from the result.
...
### Test
- **Theme/density coverage is named for its actual subject (#775).** The former `tests/hygiene.test.ts` is now `tests/theme-density.test.ts`; no current config, script, workflow, or non-historical documentation reference names the old filename.
