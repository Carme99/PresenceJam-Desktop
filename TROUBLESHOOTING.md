...
**Fix / Workaround:**
This is a known architectural limitation: the filter screens the formatted status string (see [the polling page](./docs/architecture/polling.md#profanity-filter)), verified against `poll_once.rs` (filter applied at the single status-write path) and `profanity.rs`. A future release will filter raw Spotify fields before formatting.
