...
- Scope: `Presence.ReadWrite Presence.Read openid profile offline_access` (the literal `MICROSOFT_GRAPH_SCOPES` constant in `src-tauri/src/teams.rs`; `Presence.ReadWrite` covers the status write, `Presence.Read` presence gating, `profile` the object-id claim for availability sync, and `openid`/`offline_access` identity + refresh-token issuance. The unused `User.Read` scope was dropped in the #151 fix)
