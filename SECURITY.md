...
- Scope: `user-read-currently-playing user-read-playback-state user-modify-playback-state` (the literal `SPOTIFY_SCOPES` constant in `src-tauri/src/commands/spotify_auth.rs`; `user-modify-playback-state` is the write scope that powers the tray playback controls — Play/Pause, Next, Previous, device transfer)

### Microsoft Graph API

- [Microsoft Services Agreement](https://www.microsoft.com/servicesagreement/)
- [Microsoft Privacy Statement](https://privacy.microsoft.com/privacystatement/)
- Requested delegated scope set: `Presence.ReadWrite Presence.Read Calendars.ReadBasic MailboxSettings.Read openid profile offline_access` (the literal `MICROSOFT_GRAPH_SCOPES` constant in `src-tauri/src/teams.rs`). `Presence.ReadWrite` covers status writes; `Presence.Read` covers presence gating; `Calendars.ReadBasic` supports the upcoming-calendar gate; `MailboxSettings.Read` supports working-hours import; `profile` supplies the `oid` claim used by the `/users/{oid}` presence fallback; and `openid` plus `offline_access` provide identity and refresh-token issuance.

#### Borrowed Microsoft identity

PresenceJam's device-code flow uses client id `14d82eec-204b-4c2f-b7e8-296a70dab67e`, borrowed from the first-party **Microsoft Graph Command Line Tools** application registration. **PresenceJam does not currently use a PresenceJam-owned Microsoft Entra app registration.** The client id is a public identifier, not a client secret; PresenceJam has no Microsoft client secret or tenant credential to disclose.

Because that application identity is shared with Microsoft Graph CLI and any other consumer of the same first-party registration, Microsoft-side consent, policy, service, and revocation changes can affect PresenceJam along with those other consumers. PresenceJam's own grant may therefore be attributed to Microsoft Graph Command Line Tools, but that owner name alone does not prove that a particular grant came from PresenceJam because other consumers share the identity. Revoking PresenceJam's grant stops this flow, while changing or revoking the shared registration also affects other consumers; there is no isolated PresenceJam app-specific revocation. The issued access and refresh tokens remain stored only in the encrypted local token store described above, and credentials are never included in documentation or logs.
